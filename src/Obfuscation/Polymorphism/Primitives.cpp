...
{
    uint8_t modrm = this->Read(opAddr + 1);
    ...
    if (mod != 0x03) {
        if (rm == 0x04) {                   // SIB present
            uint8_t base = this->Read(opAddr + 2) & 0x07;
            off += 1;
...
{
    rexW = false;
    uint8_t prev = this->Read(opAddr - 1);
    if ((prev & 0xF0) == 0x40) { rexW = (prev & 0x08) != 0; prev = this->Read(opAddr - 2); }
    return prev == 0x66;
}
...
{
    uint32_t v = 0;
    for (int i = 0; i < size; ++i) v |= (uint32_t)this->Read(a + i) << (i * 8);
    return v;
}
...
{
    for (int i = 0; i < size; ++i) this->Write(a + i, (uint8_t)(v >> (i * 8)));
}
...
{
    this->Write(opAddr + 1, (uint8_t)((modrm & 0xC7) | (newReg << 3)));
}
void CPolymorphic::SetModRM(uintptr_t opAddr, uint8_t reg, uint8_t rm) 
{
    this->Write(opAddr + 1, (uint8_t)(0xC0 | (reg << 3) | rm));
}
...
void CPolymorphic::SwapRexRB(uintptr_t rexAddr, uint8_t rex) 
{
    ...
    if (rex) {
        ...
        this->Write(rexAddr, (uint8_t)((rex & 0xFA) | (b << 2) | r));
    }
#endif
}
